...
import { db } from "@/lib/db";
import { blogs, brands } from "@/lib/db/schema";
import { getAbsoluteURL } from "@/lib/utils";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // ✅ Fetch blogs and brands data
  const [blogsData, brandsData] = await Promise.all([
    db.query.blogs.findMany({
      where: eq(blogs.isPublished, true),
    }),
    db.query.brands.findMany(),
  ...
  // ✅ Add Home & Shop entries explicitly (strong SEO signals)
  ...
  // ✅ Combine and return (Home + Shop + Static + Dynamic pages)
  return [...mainPages, ...staticPages, ...blogPages, ...brandPages];
}
