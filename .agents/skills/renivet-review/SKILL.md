...
2. Locate `docs/.work-items/<ID>/work-item.yaml` and its referenced SPEC artifacts. Run `bun run governance:validate -- docs/.work-items/<ID>/work-item.yaml`, then require `task.status: READY_FOR_DEV`, `approval.state: APPROVED`, no design blockers, and a non-empty approver.
3. If the work item is absent, invalid, or not approved and `READY_FOR_DEV`, stop with `SPEC_CONTRACT_MISSING: <exact reason>`. Do not create or update review artifacts and do not reconstruct or relax the contract.
4. Resolve an explicit comparison base. Prefer PR base/head metadata when available; otherwise identify the tracked base branch and use its merge base with `HEAD`. Record the base branch, exact 40-character base/head commits, PR URL or `null`, and any uncommitted state.
...
7. Read [references/review-output.md](references/review-output.md). Write `docs/.work-items/<ID>/REVIEW.md` and replace the current `implementation_review` object in `work-item.yaml` with the normalized result. Material drift also moves `task.status` to `IN_REVIEW`, or `BLOCKED` for an unresolved Class C decision; do not alter any approved requirement, design, traceability, Critic, or approval content.
8. Run `bun run governance:validate -- docs/.work-items/<ID>/work-item.yaml`. If validation fails, correct only `REVIEW.md` or review-related YAML fields and validate again. Report the exact validator failure if a valid result cannot be produced.
