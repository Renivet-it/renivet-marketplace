...
Compared `origin/master` and commit `ef97911e0c0f3c49dc3c470be5c7d60ea2241874`. Evidence includes `src/lib/redis/methods/cart.ts`, `src/lib/redis/methods/cart-key.ts`, `src/lib/redis/methods/cart.test.ts`, and all changed cart mutation paths in `src/lib/trpc/routes/general/cart.ts`.
