...
5. Admin selects a separate commission-service HSN/SAC from the HSN master at manual quote creation. Its GST rate and HSN/SAC code are persisted on the quote/order and reused in settlement and commission documents; issuance blocks when that classification is absent.
