...
5. For every other brand, and for Terra Luna when no approved effective rule matches, return an explicit blocked/flagged result with zero payable commission. Never silently calculate or omit an unapproved commission result.
...
The requester approved the previously blocking decisions:
...
The contract is `READY_FOR_DEV`. REN-206 must remain blocked until REN-203, REN-204, and REN-205 are complete and the real-transaction validation has explicit clearance.
