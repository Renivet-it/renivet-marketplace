...
export function useMergeGuestCartOnLogin() {
    ...
    useEffect(() => {
        ...
        if (userId && guestCart.length > 0 && !mergeGuestCart.isLoading) {
...
export function useMergeGuestWishlistOnLogin() {
    ...
    const mergeGuestWishlist =
        trpc.general.users.wishlist.mergeGuestWishlist.useMutation({
            onSuccess: () => {
                clearGuestWishlist();
                if (userId) {
                    utils.general.users.wishlist.getWishlist.invalidate({
                        userId,
                    });
                }
            },
