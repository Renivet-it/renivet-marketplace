...
class OrderQuery {
    ...
    async getOrders({
        ...
        // Filter by pickup scheduled status
        if (pickupScheduled !== undefined) {
            // When pickupScheduled = true, also require shipment status = 'pending'
            const shipmentCondition = pickupScheduled
                ? and(
            ...
            whereConditions.push(
                inArray(
                    orders.id,
                    db
                        .select({ id: orderShipments.orderId })
                        .from(orderShipments)
        ...
        // Filter by RTO status (return or replacement label generated)
        if (rto === true) {
            whereConditions.push(
                inArray(
                    orders.id,
            ...
            whereConditions.push(
                inArray(
                    ...
                    db
                        .select({ id: orderShipments.orderId })
                        .from(orderShipments)
                        .where(eq(orderShipments.status, shipmentStatus))
                )
            );
        }

        // Shipped tab: orders.status = 'processing' OR orderShipments.status = 'in_transit'
        if (shippedOrProcessing === true) {
            whereConditions.push(
                or(
