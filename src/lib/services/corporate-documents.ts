...
export const corporateDocumentService = {
    ...
    async issueVendorPurchaseOrder(actorUserId: string, input: unknown) {
        ...
        const [settings, brandDetails, incomingPo, receiptVoucher] =
            await Promise.all([
        ...
        if (!incomingPo || incomingPo.status !== "po_accepted") {
            throw new TRPCError({
                code: "PRECONDITION_FAILED",
                message:
                    "Approve the corporate customer's purchase order before issuing the Renivet PO",
            });
        }
        ...
        const gstPaise = Math.round(
            (taxableValuePaise * parsed.gstRateBps) / 10_000
        );
