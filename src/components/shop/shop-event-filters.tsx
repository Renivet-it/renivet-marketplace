...
function ShopEventFiltersSection({
    ...
    const [priceRange, setPriceRange] = useState<number[]>([
        ...
        queryStates.maxPrice && queryStates.maxPrice < SHOP_PRICE_FILTER_MAX
            ? queryStates.maxPrice
            : SHOP_PRICE_FILTER_MAX,
    ]);
    ...
    const handlePriceCommit = (values: number[]) => {
        setQueryStates({
            ...
            maxPrice: values[1] >= SHOP_PRICE_FILTER_MAX ? null : values[1],
            page: 1, // Reset to page 1 when price changes
        });
    };

    const handleBrandChange = (options: any[]) => {
        const newBrandIds = options
            .map(
                (option) =>
        ...
        <div className={cn("", className)} {...props}>
            ...
            <div className="space-y-1">
                ...
                <MultipleSelectorGeneral
                    ...
                    value={brandsMeta
                        .filter((brand) =>
                            queryStates.brandIds.includes(brand.id)
                        )
                        .map((brand) => ({
                            label: brand.name,
                            value: brand.slug,
