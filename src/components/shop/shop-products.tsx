...
export function ShopProducts({
  ...
  const [subCategoryId] = useQueryState("subcategoryId", { defaultValue: "" });
  const [productTypeId] = useQueryState("productTypeId", { defaultValue: "" });
  ...
  if (isFetching && !visibleProducts.length) {
    return (
      <div className="flex justify-center py-10">
        <Icons.Loader2 className="animate-spin size-6" />
      </div>
    );
  }
