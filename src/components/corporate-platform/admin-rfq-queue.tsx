...
export function AdminRfqQueue({
            ...
            onSuccess: async (quote) => {
                ...
                toast.success(
                    quote.recipientRegistered
                        ...
                        : `${quote.quoteNumber} created; it will link when the customer registers`
                );
    ...
    const manualQuoteQuantity = Number(manualQuote.quantity || 0);
    const manualQuoteUnitPricePaise = Math.round(
        Number(manualQuote.unitPrice || 0) * 100
    );
    const manualQuoteCustomizationPaise = Math.round(
        Number(manualQuote.customizationCost || 0) * 100
    );
    const manualQuoteSubtotalPaise =
        manualQuoteQuantity * manualQuoteUnitPricePaise;
    const manualQuoteTaxPaise = Math.round(
        ((manualQuoteSubtotalPaise + manualQuoteCustomizationPaise) *
            Number(manualQuote.gstPercent || 0)) /
            100
    );
    const manualQuoteTotalPaise =
        manualQuoteSubtotalPaise +
        manualQuoteCustomizationPaise +
        manualQuoteTaxPaise;
    const manualQuoteGstPercent = Number(manualQuote.gstPercent || 0);
    const manualQuoteAdvancePercent = Number(manualQuote.advancePercent || 0);
    const canCreateManualQuote =
        manualQuote.companyName.trim().length >= 2 &&
        manualQuote.contactPerson.trim().length >= 2 &&
        /^\S+@\S+\.\S+$/.test(manualQuote.email.trim()) &&
        manualQuote.phone.trim().length >= 8 &&
        Boolean(manualQuote.brandId) &&
        manualQuoteQuantity > 0 &&
        manualQuoteUnitPricePaise > 0 &&
        manualQuoteCustomizationPaise >= 0 &&
        Number.isFinite(manualQuoteGstPercent) &&
        manualQuoteGstPercent >= 0 &&
                ...
                <DialogContent className="max-h-[90vh] max-w-3xl gap-3 overflow-y-auto p-5">
                    <DialogHeader className="text-left">
                        ...
                        <DialogDescription className="text-xs">
                            For phone or email enquiries. Registered customers
                            see it immediately; otherwise it is claimed when
                            this email registers.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <LabelledInput
                            ...
                            value={manualQuote.contactPerson}
                    ...
                    <div className="grid gap-3 border-t border-slate-100 pt-3 sm:grid-cols-2 lg:grid-cols-4">
                        <CompactSelect
                            ...
                            options={brandOptions.map((brand) => ({
                                value: brand.id,
                                label: `${brand.name}${brand.isActive ? "" : " (Inactive)"}`,
                            }))}
                        />
                        <CompactSelect
                            label="Product type"
                            value={manualQuote.productTypeId}
                            ...
                            options={(orderConfig?.productTypes ?? []).map(
                        ...
                        <CompactSelect
                            ...
                            options={(orderConfig?.gsmOptions ?? []).map(
                                (item) => ({
                                    value: item.id,
                                    label: item.label,
                                })
                        ...
                        <CompactSelect
                            label="Fabric"
                            value={manualQuote.fabricCompositionId}
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        />
                        <LabelledInput
                            label="Customization (INR)"
                            placeholder="0"
                            type="number"
                            value={manualQuote.customizationCost}
                            onChange={(customizationCost) =>
                                setManualQuote((current) => ({
                                    ...current,
                                    customizationCost,
                                }))
                        ...
                        <LabelledInput
                            ...
                            placeholder="30"
                            type="number"
                            value={manualQuote.advancePercent}
                        ...
                        <div className="sm:col-span-2">
                            <LabelledInput
                                label="Commercial notes"
                                placeholder="Quote terms or enquiry context"
                                value={manualQuote.comments}
                                onChange={(comments) =>
                                    setManualQuote((current) => ({
                                        ...current,
                                        comments,
                    ...
                    <div className="grid grid-cols-2 gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs sm:grid-cols-4">
                        <ManualQuoteTotal
                            label="Subtotal"
                            value={manualQuoteSubtotalPaise}
                        />
                        <ManualQuoteTotal
                            label="Customization"
                            value={manualQuoteCustomizationPaise}
                        />
                        <ManualQuoteTotal
                            label="Tax"
                            value={manualQuoteTaxPaise}
                        />
                        <ManualQuoteTotal
                            label="Total"
                            value={manualQuoteTotalPaise}
                            strong
                        />
                    </div>
                    ...
                    <DialogFooter className="gap-2 sm:space-x-0">
                        ...
                        <Button
                            ...
                            className="h-9 text-xs"
                            disabled={
                                !canCreateManualQuote ||
                                createManualQuote.isPending
                            }
                            onClick={() =>
                                createManualQuote.mutate({
                                    companyName: manualQuote.companyName.trim(),
                                    contactPerson:
                                    ...
                                    gsmOptionId:
                                        manualQuote.gsmOptionId || null,
                                    fabricCompositionId:
                                        manualQuote.fabricCompositionId || null,
                                    quantity: manualQuoteQuantity,
                                    unitPricePaise: manualQuoteUnitPricePaise,
                                    customizationCostPaise:
                                        manualQuoteCustomizationPaise,
                                    gstPercent: manualQuoteGstPercent,
                                    advancePercent: manualQuoteAdvancePercent,
                                    validUntil: manualQuote.validUntil || null,
                                    comments:
                                        manualQuote.comments.trim() || null,
                                })
                            }
                        >
                            {createManualQuote.isPending
