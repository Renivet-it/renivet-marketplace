...
type PersistedSwipeState = {
    goneProductIds: string[];
    hasInteracted: boolean;
    swipeDir: Record<string, SwipeDirection>;
};
...
const SwipeHint = () => (
    <div className="pointer-events-none absolute inset-0 z-30 flex flex-col items-center justify-center gap-3">
        <div className="flex items-center gap-6">
            ...
            <div className="flex h-14 w-14 animate-bounce items-center justify-center rounded-full border-2 border-white/40 bg-white/20 shadow-lg backdrop-blur-md">
                <svg
...
export function SwipeCard({ products, userId }: SwipeableProductCardProps) {
    ...
    const storageKey = userId
        ? `renivet_discover_swipe_state:${userId}`
        : "renivet_discover_swipe_state:guest";

    const [goneCards, setGoneCards] = useState<Set<string>>(new Set());
    const [hasInteracted, setHasInteracted] = useState(false);
    const [swipeDir, setSwipeDir] = useState<Record<string, SwipeDirection>>(
        {}
    );
    ...
    useEffect(() => {
        ...
        try {
            ...
            const validProductIds = new Set(
                filteredProducts.map((item) => item.product.id)
            );
            const nextGone = new Set(
                (parsed.goneProductIds ?? []).filter((id) =>
                    validProductIds.has(id)
                )
            );
            const nextDirections = Object.fromEntries(
                Object.entries(parsed.swipeDir ?? {}).filter(([id]) =>
                    validProductIds.has(id)
                )
            ) as Record<string, SwipeDirection>;

            setGoneCards(nextGone);
            setSwipeDir(nextDirections);
            setHasInteracted(Boolean(parsed.hasInteracted || nextGone.size > 0));

            window.setTimeout(() => {
                api.start((i) =>
                    nextGone.has(filteredProducts[i]?.product.id)
    ...
    }, [api, filteredProducts, storageKey]);

    useEffect(() => {
        if (typeof window === "undefined") return;

        const state: PersistedSwipeState = {
            goneProductIds: [...goneCards],
            hasInteracted,
            swipeDir,
        };

        window.localStorage.setItem(storageKey, JSON.stringify(state));
    }, [goneCards, hasInteracted, storageKey, swipeDir]);
    ...
    const handleSwipe = async (index: number, dir: number) => {
        ...
        if (!product || goneCards.has(product.id)) return;

        setHasInteracted(true);
        setSwipeDir((prev) => ({
            ...prev,
            [product.id]: dir === 1 ? "right" : "left",
        }));
        setGoneCards((prev) => new Set(prev).add(product.id));
    ...
    const bind = useDrag(
        ({
            ...
            const productId = filteredProducts[index]?.product.id;
            if (!productId || goneCards.has(productId)) return;
            setHasInteracted(true);
    ...
    const allGone =
        goneCards.size === filteredProducts.length && filteredProducts.length > 0;
    const showGuide =
        !hasInteracted &&
        goneCards.size === 0 &&
                ...
                <div className="flex flex-col items-center gap-10 md:flex-row md:items-center md:gap-16">
                    <div className="relative flex h-[420px] w-full max-w-[340px] shrink-0 items-center justify-center self-center md:h-[500px] md:max-w-[400px]">
                        <div className="pointer-events-none absolute inset-x-0 top-1/2 z-40 flex -translate-y-1/2 items-center justify-between px-3">
                            <span
                                className="rounded-full bg-black/70 px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-white opacity-0 transition-opacity duration-150 data-[show=true]:opacity-100"
                                data-show={String(hasInteracted)}
                            >
                                Skip
                            </span>
                            <span
                                className="rounded-full bg-[#c8a96e]/90 px-3 py-1 text-[9px] font-bold uppercase tracking-widest text-white opacity-0 transition-opacity duration-150 data-[show=true]:opacity-100"
                                data-show={String(hasInteracted)}
                            >
                                Save
                            </span>
                        ...
                        {allGone && (
                            <div className="flex flex-col items-center gap-4 text-center">
                                ...
                                <Link
                                    ...
                                    className="mt-2 inline-flex items-center gap-1.5 rounded-sm border border-gray-900 bg-gray-900 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.12em] text-white transition hover:bg-transparent hover:text-gray-900"
                                >
                                    View Wishlist
                                </Link>
                            </div>
                        ...
                        {springs.map(({ x, y, rot, scale }, i) => {
                            ...
                            if (!product || goneCards.has(product.id)) return null;
                            ...
                            const comparePrice =
                                originalPrice && originalPrice > rawPrice
                                    ...
                                    : null;
                            const activeIndex = filteredProducts.findIndex(
                                (entry) => !goneCards.has(entry.product.id)
                            );
                                ...
                                <animated.div
                                    ...
                                    style={{
                                        transform: interpolate(
                                            [x, y, rot, scale],
                                            (nextX, nextY, nextRot, nextScale) =>
                                                `translate3d(${nextX}px,${nextY}px,0) rotate(${nextRot}deg) scale(${nextScale})`
                                        ),
                                        zIndex: i,
                                        touchAction: "none",
                                    }}
                                    ...
                                    <div className="flex items-center justify-between px-4 py-3.5">
                                        ...
                                        <div className="flex items-center gap-2.5">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    void handleSwipe(i, -1);
                                                }}
                                                className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-gray-200 bg-white text-gray-500 shadow-sm transition-all hover:scale-110 hover:border-gray-900 hover:text-gray-900 active:scale-95"
                                                title="Skip"
                                            >
                                                <svg
                                                    className="h-4 w-4"
                                            ...
                                            <button
                                                ...
                                                className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-[#c8a96e] bg-[#c8a96e] text-white shadow-sm transition-all hover:scale-110 hover:bg-[#b8965e] active:scale-95"
                    ...
                    <div className="flex w-full flex-col gap-4">
                        ...
                        >
                            ...
                            <div className="relative my-4">
                                <h3 className="font-playfair text-[32px] font-normal leading-[1.15] text-gray-900 md:text-[38px]">
                                    Got
                                    <br />
                                    <em className="not-italic text-[#5a7020]">
                                        curiosity?
                                    </em>
                                </h3>
                        ...
                        <div className="flex items-stretch gap-3">
                            ...
                            <div className="group/tile relative flex flex-1 cursor-default flex-col items-center gap-1.5 border border-[#c5d08a] bg-[#f4f6ec] py-4 transition-all duration-200 hover:border-[#a0b84a] hover:shadow-sm">
                                ...
                                <p className="text-[9px] font-bold uppercase tracking-wider text-[#4a5a1a]">
                                    Save
                                </p>
                                <p className="text-[8px] text-[#7a9030]">
                                    swipe right
                                </p>
                            </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="h-0.5 w-full overflow-hidden bg-gray-100">
                                <div
                                    ...
                                    style={{
                                        width: `${(goneCards.size / filteredProducts.length) * 100}%`,
                                    }}
                                />
                            </div>
                        </div>
                        ...
                        >
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-700">
                                        Saved Items
                                    </p>
                                    <p className="text-[9px] text-gray-400">
                                        {goneCards.size > 0
                                            ? `${[...goneCards].filter((id) => swipeDir[id] === "right").length} saved · ${[...goneCards].filter((id) => swipeDir[id] === "left").length} skipped`
                                            : "Start swiping"}
                                    </p>
                                </div>
                            </div>
                            <svg
                                className="h-3.5 w-3.5 text-gray-300 transition-all duration-200 group-hover:translate-x-1 group-hover:text-[#5a7020]"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                strokeWidth={2}
