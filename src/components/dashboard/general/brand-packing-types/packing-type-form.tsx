...
export function BrandProductPackingForm({
    ...
    const { data: brands } = trpc.general.brands.getBrands.useQuery({
        page: 1,
        limit: 1000,
    });
    ...
    const { data: packingTypes } = trpc.general.packingTypes.getAll.useQuery({
        page: 1,
        limit: 100,
    });

    const { data } = trpc.general.brandProductTypePacking.getById.useQuery(
        { id: id! },
        { enabled: !!id }
    );
    ...
    const create = trpc.general.brandProductTypePacking.create.useMutation({
        onSuccess: () => {
    ...
    const update = trpc.general.brandProductTypePacking.update.useMutation({
        onSuccess: () => {
    ...
    return (
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Select
                ...
                onValueChange={(v) => form.setValue("packingTypeId", v || null)}
            >
                <SelectTrigger>
            ...
            <label className="flex items-center gap-2">
                <Checkbox
                    ...
                    onCheckedChange={(v) => form.setValue("shipsInOwnBox", !!v)}
                />
                Ships in Own Box
            </label>
            ...
            <label className="flex items-center gap-2">
                <Checkbox
                    ...
                    onCheckedChange={(v) => form.setValue("canOverride", !!v)}
                />
                Can Override
            </label>
