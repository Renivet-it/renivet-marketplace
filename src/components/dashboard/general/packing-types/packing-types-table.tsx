...
const columns: ColumnDef<TablePackingType>[] = [
    ...
    // },
    {
        ...
        cell: ({ row }) => {
            ...
            if (!hasValidSize) {
                return <span className="text-muted-foreground">Box Size</span>;
            }
    ...
    {
        id: "actions",
        cell: ({ row }) => <PackingTypeAction packingType={row.original} />,
    },
];
...
export function PackingTypesTable({ initialData }: PageProps) {
    ...
    const { data: queryData } = trpc.general.packingTypes.getAll.useQuery(
        {
            page,
            limit,
