...
const handleDownloadBrandPDF = async () => {
  ...
  const addressParts = [
    firstOrder?.items?.[0]?.product?.brand?.confidential?.warehouseAddressLine1 || "",
    firstOrder?.items?.[0]?.product?.brand?.confidential?.warehouseAddressLine2 || "",
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    firstOrder?.items?.[0]?.product?.brand?.confidential?.gstin ?? "N/A", 
    leftBoxX + 110, 
    phoneY + 20
  );
