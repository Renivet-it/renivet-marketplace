...
export function OrdersTable({ initialData, brandData }: PageProps) {
    ...
    const {
    ...
    } = trpc.general.orders.getOrders.useQuery(
        {
            ...
            startDate: startDate ? format(startDate, "yyyy-MM-dd") : undefined,
            endDate: endDate ? format(endDate, "yyyy-MM-dd") : undefined,
            // Map status filter to the appropriate order status
            status:
                statusFilter === "delivered"
                    ? "delivered"
                    : statusFilter === "cancelled"
                      ? "cancelled"
                      : statusFilter === "ready_to_pickup"
                        ? "pending"
                        : statusFilter === "pickup_scheduled"
                          ? "pending"
                          : undefined,
            // Ready to Pickup = pending status with pickupScheduled = false
            pickupScheduled:
                statusFilter === "ready_to_pickup"
                    ? false
                    : statusFilter === "pickup_scheduled"
                      ? true
                      : undefined,
            // Shipped tab uses OR logic: orders.status = 'processing' OR orderShipments.status = 'in_transit'
            shippedOrProcessing: statusFilter === "shipped" ? true : undefined,
            // RTO filter
            rto: statusFilter === "rto" ? true : undefined,
        },
        {
            initialData,
    ...
    return (
        <div className="space-y-6 rounded-lg border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <Tabs
                ...
                className="w-full"
            >
                <TabsList className="grid w-full grid-cols-4 md:grid-cols-7 lg:inline-grid lg:w-auto">
