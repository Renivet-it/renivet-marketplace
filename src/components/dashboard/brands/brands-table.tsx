...
import { BrandWithOwnerAndMembers } from "@/lib/validations";
import {
    ColumnDef,
    ColumnFiltersState,
    getCoreRowModel,
...
export type TableBrand = BrandWithOwnerAndMembers & {
    ownerName: string;
    memberCount: number;
};
...
export function BrandsTable({ initialData }: PageProps) {
    ...
    } = trpc.brands.getBrands.useQuery(
        { page, limit, search },
        { initialData }
    );
