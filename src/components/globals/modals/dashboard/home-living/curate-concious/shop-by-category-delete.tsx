...
export function ShopByCategoryDeleteModal({
    ...
    const { refetch } =
        trpc.general.content.homeAndLivingCurateConciousRouter.getwomenHomeBanners.useQuery(
            {
                page,
                limit,
            }
    ...
    const { mutate: deleteShopByCategory, isPending: isDeleting } =
        trpc.general.content.homeAndLivingCurateConciousRouter.deleteWomenBanner.useMutation(
            {
                onMutate: () => {
