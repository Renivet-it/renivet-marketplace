...
export function ShopByCategoryDeleteModal({
    ...
    const { refetch } =
        trpc.general.content.homeAndLivingNewCollectionRouter.getwomenHomeBanners.useQuery(
            {
                page,
                limit,
            }
    ...
    const { mutate: deleteShopByCategory, isPending: isDeleting } =
        trpc.general.content.homeAndLivingNewCollectionRouter.deleteWomenBanner.useMutation(
            {
                onMutate: () => {
