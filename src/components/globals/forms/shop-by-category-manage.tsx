...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.homeShopByCategories.createHomeShopByCategory.useMutation();
    const { mutateAsync: updateAdAsync } =
        trpc.general.content.homeShopByCategories.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating shop by category...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("Shop by category created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/shop-by-category");
            setPreview(null);
            setFile(null);
        },
        onError: (err, _, ctx) => {
