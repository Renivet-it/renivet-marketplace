...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.womenStyleDirectoryRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =

    // @ts-ignore
        trpc.general.content.womenStyleDirectoryRouter.updateWomenBanner.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating style directory...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("style directory created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/women-section/style-directory");
