...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.womenElavateLooksSectionRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =
    // @ts-ignore
        trpc.general.content.womenElavateLooksSectionRouter.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating Elevate Looks...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("Elevate Looks created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/women-section/elevate-looks");
