...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.womenDiscountOfferRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =
    // @ts-ignore
        trpc.general.content.womenDiscountOfferRouter.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating Discount Offer...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            ...
            router.push("/dashboard/general/women-section/discount-offer");
