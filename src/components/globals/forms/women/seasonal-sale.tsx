...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.womenBrandSeasonalSaleRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =

    // @ts-ignore
        trpc.general.content.womenBrandSeasonalSaleRouter.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating Seasonal sale...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("Seasonal sale created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/women-section/seasonal-sale");
