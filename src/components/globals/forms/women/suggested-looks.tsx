...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.womenSuggestedLooksSectionRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =

    // @ts-ignore
        trpc.general.content.womenSuggestedLooksSectionRouter.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating Suggested Looks...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("Suggested Looks created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/women-section/suggested-looks");
