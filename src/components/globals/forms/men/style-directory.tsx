...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.menStyleDirectoryRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =
    // @ts-ignore

        trpc.general.content.menStyleDirectoryRouter.updateWomenBanner.useMutation();
    ...
    const { mutate: updateBrandProduct, isPending: isUpdating } = useMutation({
        ...
        onSuccess: (_, __, { toastId }) => {
            ...
            router.push("/dashboard/general/men-section/style-directory");
            setPreview(null);
            setFile(null);
        },
        onError: (err, _, ctx) => {
