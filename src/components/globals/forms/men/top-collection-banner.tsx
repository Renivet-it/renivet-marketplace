...
export function ShopByCategoryManageForm({ shopByCategory }: PageProps) {
    ...
    const { mutateAsync: createAdAsync } =
        trpc.general.content.menTopcollectionBannerRouter.createWomenBanner.useMutation();
    const { mutateAsync: updateAdAsync } =
    // @ts-ignore

        trpc.general.content.menTopcollectionBannerRouter.updateHomeShopByCategory.useMutation();

    const { mutate: createBrandProduct, isPending: isCreating } = useMutation({
        onMutate: () => {
            const toastId = toast.loading("Creating Top Collection...");
            return { toastId };
        },
        ...
        onSuccess: (_, __, { toastId }) => {
            toast.success("Top Collection created successfully", {
                id: toastId,
            });
            router.push("/dashboard/general/men-section/top-collection");
