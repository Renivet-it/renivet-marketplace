...
export function EmailCompletionCard() {
    ...
    const verifyCode = async (event: FormEvent) => {
        ...
        try {
            ...
            const result = await addressToVerify.attemptVerification({ code });
            ...
            if (!sync.ok)
                throw new Error("Your email was verified, but profile sync failed. Please refresh and try again.");
            setStep("complete");
        } catch (value) {
            showError(value);
        } finally {
