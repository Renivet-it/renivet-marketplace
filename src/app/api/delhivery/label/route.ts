    ...
    try {
        ...
        if (!wbn) {
            return NextResponse.json(
                { success: false, message: "WBN is required" },
                { status: 400 }
            );
        }

        const denied = await requireShipmentLogisticsAccess({ waybill: wbn });
        ...
        if (pdfHeader !== "%PDF") {
            ...
            console.error(
                "❌ Buffer as text (first 200 chars):",
                buffer.toString("utf8", 0, 200)
            );
        }
        ...
        return NextResponse.json(
            {
                success: false,
                message: err?.message || "Failed to download Delhivery label",
            },
            { status: 500 }
        );
    }
