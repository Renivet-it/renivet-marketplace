...
export async function POST(req: Request) {
  try {
    ...
    const price = convertPaiseToRupees(order.amount);

    // ------------------------------------------------------
    // 🟨 CREATE HTML TEMPLATE (Single row)
    // ------------------------------------------------------
    const html = `
<html>
<head>
  ...
  <div class="invoice-box">
    ...
    <div class="flex-row">
      <div class="box">
        ...
        ${order.brand.name}<br>
        ${order.brand.confidential.addressLine1}<br>
        GST: ${order.brand.confidential.gstin}
      </div>
...
<p><strong>Order Date:</strong> ${new Date(order.date).toLocaleDateString()}</p>
    ...
    <table>
      ...
      <tr>
        ...
        <td>1</td>
        <td>₹${convertPaiseToRupees(order.amount)}</td>
        <td>₹${convertPaiseToRupees(order.amount)}</td>
      </tr>
    </table>
    ...
    <table class="summary-table">
      ...
      <tr>
        ...
        <td><strong>₹${convertPaiseToRupees(order.amount)}</strong></td>
      </tr>
    </table>
