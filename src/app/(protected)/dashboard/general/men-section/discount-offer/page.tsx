...
} from "@/components/dashboard/general/men/discount-offers/shop-by-categories-table";
...
export const metadata: Metadata = {
    ...
    description: "Manage the platform's Discount Offers",
};
            ...
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    ...
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s Discount Offers
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/men-section/discount-offer/new">
                        <Icons.PlusCircle className="size-5" />
                        New women Discount Offers
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getDiscountOfferSections({
            limit,
            page,
        }),
    ]);
