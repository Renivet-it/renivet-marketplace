...
} from "@/components/dashboard/general/men/fresh-ink-collection/shop-by-categories-table";
...
export const metadata: Metadata = {
    title: "Mood Board",
    description: "Manage the platform's Mood Board",
};
            ...
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    ...
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s Mood Board
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/men-section/fresh-ink-collection/new">
                        <Icons.PlusCircle className="size-5" />
                        New women Mood Board
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getMenFreshInkCollection({
            limit,
            page,
        }),
    ]);
