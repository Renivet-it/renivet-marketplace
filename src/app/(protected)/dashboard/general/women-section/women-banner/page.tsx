...
} from "@/components/dashboard/general/women/banners/shop-by-categories-table";
        ...
        <DashShell>
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    <h1 className="text-2xl font-bold">women home banner</h1>
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s women home banner
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/women-section/women-banner/new">
                        <Icons.PlusCircle className="size-5" />
                        New women home banner
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getAllHomeShopByCategories({
            limit,
            page,
        }),
