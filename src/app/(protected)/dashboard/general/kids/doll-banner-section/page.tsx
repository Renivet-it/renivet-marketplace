...
} from "@/components/dashboard/general/kids/doll-banner/shop-by-categories-table";
...
export const metadata: Metadata = {
    title: "New Discount Banner",
    description: "Manage the platform's New Discount Banner",
};
        ...
        <DashShell>
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    <h1 className="text-2xl font-bold">kids doll Banner</h1>
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s kids doll Banner
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/kids/doll-banner-section/new">
                        <Icons.PlusCircle className="size-5" />
                        New Doll Banner Section
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getkidDollBuyingSections({
            limit,
            page,
        }),
