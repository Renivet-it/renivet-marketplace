...
} from "@/components/dashboard/general/kids/special-care/shop-by-categories-table";
...
export const metadata: Metadata = {
    title: "Kids Special Ca",
    description: "Manage the platform's Kids Special Care",
};
        ...
        <DashShell>
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    <h1 className="text-2xl font-bold">Special Carer</h1>
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s Special Carer
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/kids/special-care/new">
                        <Icons.PlusCircle className="size-5" />
                        New Special Care Banner
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getKidsCareSections({
            limit,
            page,
        }),
