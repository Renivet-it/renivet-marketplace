...
} from "@/components/dashboard/general/home/event-section-two/shop-by-categories-table";
        ...
        <DashShell>
            <div className="flex flex-col items-center justify-between gap-4 md:flex-row md:gap-2">
                <div className="space-y-1 text-center md:text-start">
                    <h1 className="text-2xl font-bold">Event Section Banner</h1>
                    <p className="text-balance text-sm text-muted-foreground">
                        Manage the platform&apos;s Event Section Banner
                    </p>
                </div>
                ...
                >
                    <Link href="/dashboard/general/home-page/event-section-two/new">
                        <Icons.PlusCircle className="size-5" />
                        New Event Section Banne
                    </Link>
                </Button>
            </div>
...
async function ShopByCategoriesFetch({ searchParams }: PageProps) {
    ...
    const [data] = await Promise.all([
        WomenHomeSectionQueries.getHomePageEventTwoSections({
            limit,
            page,
        }),
